...
from loss.loss import CRLoss
from model.lightning import AutoEncoderTrainer
from model.utils import generate_grid, Dataset_epoch, Predict_dataset, transform_unit_flow_to_flow_cuda, \
    generate_grid_unit
...
parser.add_argument("--datapath", type=str,
                    dest="datapath",
                    default='/vol/alan/projects/oasis_learn2reg/train',
...
model_name = "OASIS_CRLoss_one_level_"
...
def train_lvl1():
    ...
    loss_similarity = CRLoss(t1_ae=t1_ae, t2_ae=t2_ae)
    ...
    # OASIS
    names = sorted(glob.glob(datapath + '/*/aligned_norm.nii.gz'))

    grid = generate_grid(imgshape_4)
    grid = torch.from_numpy(np.reshape(grid, (1,) + grid.shape)).cuda().float()

    optimizer = torch.optim.Adam(model.parameters(), lr=lr)
    model_dir = 'outputs/cLapIRN/stage'
    ...
    if load_model is True:
        model_path = "outputs/cLapIRN/LDR_LPBA_NCC_lap_share_preact_1_05_3000.pth"
        print("Loading weight: ", model_path)
        step = 3000
        model.load_state_dict(torch.load(model_path))
        temp_lossall = np.load("outputs/cLapIRN/loss_LDR_LPBA_NCC_lap_share_preact_1_05_3000.npy")
...
def train_lvl2():
    ...
    load_model = False
    if load_model is True:
        model_path = model_dir + "/LDR_LPBA_NCC_lap_share_preact_1_05_3000.pth"
        print("Loading weight: ", model_path)
        step = 3000
        model.load_state_dict(torch.load(model_path))
        temp_lossall = np.load(model_dir + "/loss_LDR_LPBA_NCC_lap_share_preact_1_05_3000.npy")
        lossall[:, 0:3000] = temp_lossall[:, 0:3000]
    ...
    while step <= iteration_lvl3:
        for X, Y in training_generator:
            ...
            if (step % n_checkpoint == 0):
                ...
                torch.save(model.state_dict(), modelname)
                np.save(model_dir + '/loss_' + model_name + "stagelvl3_" + str(step) + '.npy', lossall)
...
t1_ae_ckpt = './checkpoints/ae-mod-t1-epochs-200.ckpt-v1.ckpt'
t2_ae_ckpt = './checkpoints/ae-mod-t2-epochs-200.ckpt-v1.ckpt'
t1_ae = AutoEncoderTrainer.load_from_checkpoint(t1_ae_ckpt).to(device)
t2_ae = AutoEncoderTrainer.load_from_checkpoint(t2_ae_ckpt).to(device)
train_lvl1()
train_lvl2()
train_lvl3()
# time
